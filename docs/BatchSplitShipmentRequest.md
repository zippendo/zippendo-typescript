...
------------ | -------------
`parcelId` | string
`shipments` | [Array&lt;BatchSplitShipmentRequestShipmentsInner&gt;](BatchSplitShipmentRequestShipmentsInner.md)
`carrierId` | string
`productId` | string
...
```typescript
import type { BatchSplitShipmentRequest } from '@zippendo/sdk'
...
} satisfies BatchSplitShipmentRequest
...
const exampleParsed = JSON.parse(exampleJSON) as BatchSplitShipmentRequest
