/* tslint:disable */
/* eslint-disable */
/**
 * Zippendo Public API
 * Public API documentation for Zippendo. Authenticate using your API token (Bearer token prefixed with zipp_).  **Brands (sub-accounts).** An organization can be split into brands, each keeping its own orders, shipments and configuration separate. There are two ways to scope requests to one brand, and NEITHER changes any request body:  1. **Bind the token.** Create an API token with a `brandId` and every request it makes is confined    to that brand — reads filtered, writes stamped. This is the recommended way to give a single    brand\'s team its own credential. 2. **Send the `X-Zippendo-Brand` header.** An organization-wide token can scope an individual    request by sending the brand\'s id or slug in this header. Most SDKs let you set it once on the    client so every call inherits it.  A brand-bound token that receives an `X-Zippendo-Brand` header naming a different brand is rejected with `403 BRAND_ACCESS_DENIED` — the binding is never widened. Omit both and requests cover the whole organization, which is the behaviour of every existing token.  Records that belong to no brand carry `brandId: null`. Configuration (carriers, shipping rules, addresses) with a null brand is organization-wide and remains visible inside every brand; orders and shipments with a null brand are only visible organization-wide.  List endpoints additionally take a `?brandScope=own|shared|both` parameter to narrow further within whichever brand context already applies. `own` returns only rows assigned to that brand, and requires a brand context — a brand-bound token, a resolved brand session, or the `X-Zippendo-Brand` header above — otherwise `400`. `shared` returns only the organization-wide rows (equivalent to filtering `brandId=none`). The default, `both`, keeps the existing behaviour: a brand context sees its own rows plus the organization-wide ones. Set `X-Zippendo-Brand-Scope` as a client default to apply the same choice to every request instead of repeating the query parameter on each call — an explicit `brandScope` query parameter always wins over the header, and a blank header value is ignored.  Brands themselves are managed under the **Brands** tag. Retiring a brand is done with `POST /orgs/{orgId}/brands/{brandId}/archive` — permanent deletion is a dashboard-only action, since it is refused while any order, shipment, member or token still references the brand. Brands require a plan that includes them; creating one beyond your plan\'s limit returns `403`.
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: support@zippendo.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface GetOrder200ResponseOrderLinesInner
 */
export interface GetOrder200ResponseOrderLinesInner {
    /**
     * Stock keeping unit identifier.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    sku?: string | null;
    /**
     * Product name.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    name: string;
    /**
     * Quantity ordered.
     * @type {number}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    quantity: number;
    /**
     * Price per unit.
     * @type {number}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    unitPrice?: number | null;
    /**
     * Total price for the line.
     * @type {number}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    totalPrice?: number | null;
    /**
     * ISO 4217 currency code.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    currency?: string | null;
    /**
     * Item weight in the given unit.
     * @type {number}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    weight?: number | null;
    /**
     * Unit of the weight value.
     * @type {GetOrder200ResponseOrderLinesInnerWeightUnitEnum}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    weightUnit?: GetOrder200ResponseOrderLinesInnerWeightUnitEnum | null;
    /**
     * Platform variant identifier.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    variantId?: string | null;
    /**
     * Platform product identifier.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    productId?: string | null;
    /**
     * Product image URL.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    imageUrl?: string | null;
    /**
     * Harmonized System customs code (6-13 digits).
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    hsCode?: string | null;
    /**
     * ISO 3166-1 alpha-2 country of origin.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    countryOfOrigin?: string | null;
    /**
     * ISO 3166-2 province of origin.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    provinceOfOrigin?: string | null;
    /**
     * Item barcode (EAN/UPC).
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    barcode?: string | null;
    /**
     * Whether the item requires shipping.
     * @type {boolean}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    requiresShipping?: boolean | null;
    /**
     * Whether the item is taxable.
     * @type {boolean}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    taxable?: boolean | null;
    /**
     * Whether the item is a gift card.
     * @type {boolean}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    giftCard?: boolean | null;
    /**
     * Vendor or brand name.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    vendor?: string | null;
    /**
     * Order line ID. Present once the line is a row. Absent for jsonb-only lines during the dual-write window — do not synthesise one, or an edit would re-point packed lines.
     * @type {string}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    id?: string;
    /**
     * Quantity already allocated to outbound shipments.
     * @type {number}
     * @memberof GetOrder200ResponseOrderLinesInner
     */
    packedQuantity: number;
}


/**
 * @export
 */
export const GetOrder200ResponseOrderLinesInnerWeightUnitEnum = {
    Kg: 'kg',
    G: 'g',
    Lb: 'lb',
    Oz: 'oz'
} as const;
export type GetOrder200ResponseOrderLinesInnerWeightUnitEnum = typeof GetOrder200ResponseOrderLinesInnerWeightUnitEnum[keyof typeof GetOrder200ResponseOrderLinesInnerWeightUnitEnum];


/**
 * Check if a given object implements the GetOrder200ResponseOrderLinesInner interface.
 */
export function instanceOfGetOrder200ResponseOrderLinesInner(value: object): value is GetOrder200ResponseOrderLinesInner {
    if (!('name' in value) || value['name'] === undefined) return false;
    if (!('quantity' in value) || value['quantity'] === undefined) return false;
    if (!('packedQuantity' in value) || value['packedQuantity'] === undefined) return false;
    return true;
}

export function GetOrder200ResponseOrderLinesInnerFromJSON(json: any): GetOrder200ResponseOrderLinesInner {
    return GetOrder200ResponseOrderLinesInnerFromJSONTyped(json, false);
}

export function GetOrder200ResponseOrderLinesInnerFromJSONTyped(json: any, ignoreDiscriminator: boolean): GetOrder200ResponseOrderLinesInner {
    if (json == null) {
        return json;
    }
    return {
        
        'sku': json['sku'] === undefined ? undefined : json['sku'] === null ? null : json['sku'],
        'name': json['name'],
        'quantity': json['quantity'],
        'unitPrice': json['unitPrice'] === undefined ? undefined : json['unitPrice'] === null ? null : json['unitPrice'],
        'totalPrice': json['totalPrice'] === undefined ? undefined : json['totalPrice'] === null ? null : json['totalPrice'],
        'currency': json['currency'] === undefined ? undefined : json['currency'] === null ? null : json['currency'],
        'weight': json['weight'] === undefined ? undefined : json['weight'] === null ? null : json['weight'],
        'weightUnit': json['weightUnit'] === undefined ? undefined : json['weightUnit'] === null ? null : json['weightUnit'],
        'variantId': json['variantId'] === undefined ? undefined : json['variantId'] === null ? null : json['variantId'],
        'productId': json['productId'] === undefined ? undefined : json['productId'] === null ? null : json['productId'],
        'imageUrl': json['imageUrl'] === undefined ? undefined : json['imageUrl'] === null ? null : json['imageUrl'],
        'hsCode': json['hsCode'] === undefined ? undefined : json['hsCode'] === null ? null : json['hsCode'],
        'countryOfOrigin': json['countryOfOrigin'] === undefined ? undefined : json['countryOfOrigin'] === null ? null : json['countryOfOrigin'],
        'provinceOfOrigin': json['provinceOfOrigin'] === undefined ? undefined : json['provinceOfOrigin'] === null ? null : json['provinceOfOrigin'],
        'barcode': json['barcode'] === undefined ? undefined : json['barcode'] === null ? null : json['barcode'],
        'requiresShipping': json['requiresShipping'] === undefined ? undefined : json['requiresShipping'] === null ? null : json['requiresShipping'],
        'taxable': json['taxable'] === undefined ? undefined : json['taxable'] === null ? null : json['taxable'],
        'giftCard': json['giftCard'] === undefined ? undefined : json['giftCard'] === null ? null : json['giftCard'],
        'vendor': json['vendor'] === undefined ? undefined : json['vendor'] === null ? null : json['vendor'],
        'id': json['id'] == null ? undefined : json['id'],
        'packedQuantity': json['packedQuantity'],
    };
}

export function GetOrder200ResponseOrderLinesInnerToJSON(json: any): GetOrder200ResponseOrderLinesInner {
    return GetOrder200ResponseOrderLinesInnerToJSONTyped(json, false);
}

export function GetOrder200ResponseOrderLinesInnerToJSONTyped(value?: GetOrder200ResponseOrderLinesInner | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'sku': value['sku'],
        'name': value['name'],
        'quantity': value['quantity'],
        'unitPrice': value['unitPrice'],
        'totalPrice': value['totalPrice'],
        'currency': value['currency'],
        'weight': value['weight'],
        'weightUnit': value['weightUnit'],
        'variantId': value['variantId'],
        'productId': value['productId'],
        'imageUrl': value['imageUrl'],
        'hsCode': value['hsCode'],
        'countryOfOrigin': value['countryOfOrigin'],
        'provinceOfOrigin': value['provinceOfOrigin'],
        'barcode': value['barcode'],
        'requiresShipping': value['requiresShipping'],
        'taxable': value['taxable'],
        'giftCard': value['giftCard'],
        'vendor': value['vendor'],
        'id': value['id'],
        'packedQuantity': value['packedQuantity'],
    };
}

